...
const options = {
    ...
    title: 'Success',
    message: 'Created!',
    detail: 'Successfully created site.',
};
...
function updateInfo(callback, newName, newAddress, newZip, myManager, openCheck, error) {
  ...
  $queryUpdate = 'CALL create_site("'+newName+'", "'+newZip+'", "'+newAddress+'", "'+myManager+'", "'+openCheck+'")';
  //ipc.send("error-log", $queryTransits);
  connection.query($queryUpdate, function(err, rows, result) {
      if(err) {
        ipc.send("error-log", err);
        if (err.code === 'ER_DUP_ENTRY') {
           dialog.showErrorBox('Duplicate site name.', 'Make sure site name is unique.')
           error = true;
        }
